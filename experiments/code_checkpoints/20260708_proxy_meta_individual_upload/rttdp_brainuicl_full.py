...
def materialize_proxy_meta_subject(args, subject, num, student_blocks, teacher_blocks, state):
    ...
    replay_loader = DataLoader(SequenceDataset(args.train_paths), batch_size=args.batch, shuffle=True, num_workers=args.num_worker)
    ...
    for eog, eeg, _labels in new_loader:
        ...
        for row in range(eog_adv.shape[0]):
            ...
            poisoned_x = torch.cat((eog_adv[row], eeg_adv[row]), dim=1).detach().cpu().numpy().astype(np.float32)
    ...
    print(
        f"[{args.variant}] Materialized poisoned upload for Subject {subject}: {len(poisoned_data_paths)} sequences",
        flush=True,
    )
    return poisoned_data_paths, label_paths
...
def run_variant(base_args, variant, attack_mode, new_order):
    ...
    for num, subject in enumerate(new_order, start=1):
        ...
        if use_individual_proxy_upload(args):
            args.uploaded_subject_paths[int(subject)] = materialize_proxy_meta_subject(
                args,
                subject,
                num,
                blocks,
