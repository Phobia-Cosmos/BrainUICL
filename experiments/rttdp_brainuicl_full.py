...
def make_new_loader(args, subject, is_buffer, shuffle):
    new_paths = subject_paths(args.data_root, subject)
    if is_buffer:
...
def is_input_attack(attack_mode):
    return attack_mode.startswith("pgd") or attack_mode.startswith("stealth_")
...
def buffer_single_merge(args, new_task_id, num, tmp_blocks, attack_state):
    new_paths = subject_paths(args.data_root, new_task_id)
    ...
    for eog, eeg, _labels in new_loader:
        ...
        if args.store_poisoned_buffer and is_input_attack(args.attack_mode):
...
def run_variant(base_args, variant, attack_mode, new_order):
    ...
    for num, subject in enumerate(new_order, start=1):
        ...
        new_loader = make_new_loader(args, subject, is_buffer=False, shuffle=True)
        ...
        new_eval_loader = make_new_loader(args, subject, is_buffer=False, shuffle=False)
